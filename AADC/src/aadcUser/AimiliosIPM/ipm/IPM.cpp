...
{
	cuda::Stream stream;
	input.upload(_inputImg, stream);
	output.upload(_dstImg, stream);
	gpuMapX.upload(m_mapX);
	gpuMapY.upload(m_mapY);

	// Generate IPM image from src
	cv::cuda::remap(input, output, gpuMapX, gpuMapY, INTER_LINEAR, _borderMode);//, BORDER_CONSTANT, Scalar(0,0,0,0));

	output.download(_dstImg, stream);
}
